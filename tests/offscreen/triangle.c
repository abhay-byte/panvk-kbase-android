...
#define G(what, name)                                                       \
   ...
   uint8_t *px = rp_;
   /* Vulkan viewport: NDC(-1,-1) maps to framebuffer row 0 (first row in
    * CopyImageToBuffer order). Triangle covers NDC x/y in [-1,-0.25], i.e.
    * rows/cols 0..16. Interior (8,8) must be red; (55,55) stays clear. */
   uint8_t *in_px = px + (8 * W + 8) * 4;
   uint8_t *clear_px = px + (55 * W + 55) * 4;
