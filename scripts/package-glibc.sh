...
meta = {"schemaVersion":1,"name":"PanVK Kbase G615","author":"panvk-kbase-android",
 "packageVersion":"0.1.0-beta.2","vendor":"Mesa","driverVersion":"Mesa 26.3.0-devel",
...
(cd "$STAGE" && zip -q "$OUT" libvulkan_panfrost.so panfrost_icd.aarch64.json meta.json MANIFEST.json SHA256SUMS.txt SOURCE.json VALIDATION.json runtime-feature-matrix.json)
